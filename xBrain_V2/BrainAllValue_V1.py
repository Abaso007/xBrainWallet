...
def MmDrza():
    ...
    for i in range(len(mylist)):
        ...
        if int(bal) > 0:
            ...
            val = xVol1
            console.print(
                f'[light_green]Scan:[white]{count}[light_green] Tx:[white]{str(w)}[green] Rich:[white on blue]{str(s)}[light_green] Add:[blue]{str(address)}[light_green]  TXID:[white]{bal}[white on green]  Balance:[white on red]{val}'
            )
            w += 1
            if val != ifbtc:
                s += 1
                console.print(
                    f'[light_goldenrod1]Scan:[white]{count}[light_goldenrod1n] Tx:[white]{w}[light_goldenrod1] Rich:[white on blue]{s}[light_goldenrod1] Add:[blue]{str(address)}[light_goldenrod1]  TXID:[white]{bal}[white on green]  Balance:[white on blue]{val}'
                )
                with open(u"BrainWalletWinnerDetails.txt" , "a") as f:
            ...
            console.print(
                f'[gold1 on grey5]Scan:[white]{count}[gold1] Tx:[white]{str(w)}[green] Rich:[white on blue]{str(s)}[/][yellow] Add:[green1]{str(address)}[red1]  TXID:[white]{bal}[gold1]  Passphars:[white]{str(passphrase)}'
            )
