...
for i in range(len(mylist)):
    ...
    txid = xBal(address)
    print(Back.RED, Fore.WHITE, 'SCAN:', z, '  WIN:', w, ' WithVol:', s)
    print(Back.WHITE, Fore.RED, address, '           TXiD No:', txid)
    print(Back.RED, Fore.WHITE, private_key)
    z += 1
    if int(txid) > 0:
        w += 1
        urlblock = f"https://bitcoin.atomicwallet.io/address/{address}"
        ...
        print(Back.WHITE, Fore.BLACK, address, '           TXiD No:', txid)
        print(Back.YELLOW, Fore.BLACK, private_key)
        if xVol != ifbtc:
            s += 1
            print(
                Back.BLUE,
